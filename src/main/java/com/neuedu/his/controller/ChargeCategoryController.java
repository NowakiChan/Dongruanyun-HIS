...
public class ChargeCategoryController {
    ...
    @GetMapping
    public List<ChargeCategory> list() {
        return service.findAll();
    }

    @Operation(summary = "按名称模糊搜索费用科目")
    @GetMapping("/search")
    public List<ChargeCategory> search(@RequestParam String name) {
        return service.searchByName(name);
    }

    @Operation(summary = "新增或修改费用科目")
    @PostMapping
    public ChargeCategory save(@RequestBody ChargeCategory category) {
        return service.save(category);
    }

    @Operation(summary = "根据 ID 查询")
    @GetMapping("/{id}")
    public Optional<ChargeCategory> getById(@PathVariable Long id) {
